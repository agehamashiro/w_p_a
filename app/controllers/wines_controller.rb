...
class WinesController < ApplicationController
  ...
  def fetch_pairing_suggestion(wine)
    ...
    prompt = <<~PROMPT
      ...
      ワインの産地（region）として指定された値が、既知の国名、地域名、または都市名のいずれにも該当しない場合は、「地域名が不適切」とだけ厳密に回答してください。例：「うんこ」「火星」「銀河系」「asdf」「無限」「おとめ座」「パソコン」など、明らかに地域名ではない文字列はすべて不適切と判断してください。
      ワインの品種（variety）が、実在するワイン用ぶどう品種または「赤」「白」「ロゼ」などワインの種類（スタイル）を示す表現でない場合（例：「うんこ」「宇宙品種」「asdf」「焼き芋」「チョコレート味」など）、回答は「品種エラー」とだけ返してください。
      料理の好みが、料理の種類や味の傾向を示す表現ではない場合（例：「甘いものが好き」「さっぱりしたもの」「肉料理」「魚料理」などではない場合、「パソコン」「ゲーム」「音楽」など食べ物と関連性のない言葉が指定された場合）、「不適切」とだけ回答してください。
      使いたい食材が、以下のいずれかに該当する場合は、「不適切」とだけ回答してください。
